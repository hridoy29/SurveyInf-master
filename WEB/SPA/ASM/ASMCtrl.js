app.controller("ASMCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/ASM/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        ...
        $http({
            url: '/ASM/GetDynamic?where=' + where + '&orderBy=CommentsType',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
