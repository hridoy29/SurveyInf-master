app.controller("AICCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/AIC/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        ...
        $http({
            url: '/AIC/GetDynamic?where=' + where + '&orderBy=CommentsType',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
