app.controller("CommentCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function getviewList() {
        ...
        $http({
            ...
            if (data.length) {
                ...
                var begin = ($scope.PerPage * ($scope.currentPage - 1));
                var end = begin + $scope.PerPage;
                $scope.entityListPagedComment = $scope.entityList.slice(begin, end);
            }
            else {
                $scope.lsitBlock.stop();
                //alertify.log('System could not retrive information, please refresh page', 'error', '10000');
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/Comment/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        var where = "CommentsTypeId = '" + $scope.entity.CommentsTypeId + "' AND Comments = '" + $scope.entity.Comments + "'";
        if ($scope.entity.Id > 0)
            where += " AND Id = " + $scope.entity.Id;

        $http({
            url: '/Comment/GetDynamic?where=' + where + '&orderBy=CommentsTypeId',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
                alertify.log($scope.entity.CommentsTypeId + ' already exists!', 'already', '5000');
                $('#txtFocus').focus();
            } else {
                if (trnType === 'save') {
    ...
    $scope.rowClick = function (obj) {
        $scope.entity = obj;
        $scope.commentType = obj.CommentsType;
        $scope.cmbCommentType = obj;
