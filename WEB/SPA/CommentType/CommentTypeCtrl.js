app.controller("CommentTypeCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/CommentType/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        ...
        $http({
            url: '/CommentType/GetDynamic?where=' + where + '&orderBy=CommentsType',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
