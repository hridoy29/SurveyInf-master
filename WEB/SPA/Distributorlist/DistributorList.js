app.controller("DistributorlistCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/Distributorlist/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        ...
        $http({
            url: '/Distributorlist/GetDynamic?where=' + where + '&orderBy=CommentsType',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
